...
1. **Authorize publishing.** The workflow uses npm trusted publishing (OIDC) — no secret
   ...
   Enable the **npm publish** allowed action for each publisher. Once all three are
   configured, the workflow publishes only with its GitHub Actions OIDC identity; it does
   not use an npm access token.
